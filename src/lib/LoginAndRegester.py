...
def submit(
    username, password, repassword, root: tk.Tk, froot: tk.Tk, threadQueue: Queue
):
    if username == "" or password == "":
        tk.messagebox.showinfo(title="信息", message="账号或密码为空！")
        return
...
def register(froot: tk.Tk, threadQueue: Queue):
    ...
    b1 = tk.Button(
        ...
        command=lambda: submit(
            e_name.get(), e_pwd.get(), repwd.get(), root, froot, threadQueue
        ),
    )
    b1.place(x=80, y=160)
    b2 = tk.Button(
        root, text="返回", width=10, command=lambda: getLoginMess(threadQueue, root)
    )
...
def getLoginMess(threadQueue: Queue, root: tk.Tk = None) -> None:
    ...
    b1 = tk.Button(
        ...
        text="登录",
        width=10,
        command=lambda: logic(e_name.get(), e_pwd.get(), root, threadQueue),
    )
    b1.place(x=80, y=140)
    b2 = tk.Button(
        root, text="注册", width=10, command=lambda: register(root, threadQueue)
    )
